"""
Comprehensive Test Script for Enhanced 5-Day Program System
...
class Enhanced5DayProgramTester:
    ...
    async def run_comprehensive_5day_test(self) -> Dict[str, Any]:
        ...
        test_suite_start = datetime.utcnow()
        ...
        try:
            async with self.async_session() as session:
                ...
                test1_start = datetime.utcnow()
                pattern_result = await self._test_pattern_subtype_mapping(session)
                pattern_result.execution_time = (datetime.utcnow() - test1_start).total_seconds()
                ...
                test2_start = datetime.utcnow()
                movement_result = await self._test_enhanced_movement_integration(session)
                movement_result.execution_time = (datetime.utcnow() - test2_start).total_seconds()
                ...
                test3_start = datetime.utcnow()
                spacing_result = await self._test_5day_intelligent_spacing(session)
                spacing_result.execution_time = (datetime.utcnow() - test3_start).total_seconds()
                ...
                test4_start = datetime.utcnow()
                rotation_result = await self._test_primary_region_rotation(session)
                rotation_result.execution_time = (datetime.utcnow() - test4_start).total_seconds()
                ...
                test5_start = datetime.utcnow()
                program_result = await self._test_complete_5day_program(session)
                program_result.execution_time = (datetime.utcnow() - test5_start).total_seconds()
                ...
                test6_start = datetime.utcnow()
                error_result = await self._test_error_handling_integration()
                error_result.execution_time = (datetime.utcnow() - test6_start).total_seconds()
        ...
        try:
            ...
            subtype_query = await session.execute(text("""
                SELECT pattern_subtype, COUNT(*) as count,
                       primary_region, COUNT(*) as region_count
                FROM movements 
                WHERE pattern_subtype IS NOT NULL
            ...
            for row in subtype_query.fetchall():
                subtype, count, region, region_count = row
                if subtype not in subtype_distribution:
                    subtype_distribution[subtype] = {"total": 0, "regions": {}}
                subtype_distribution[subtype]["total"] += count
                subtype_distribution[subtype]["regions"][region] = region_count
            ...
            for subtype, data in subtype_distribution.items():
                if subtype in ["squat", "lunge"] and "lower body" not in data["regions"]:
                    inconsistent_mappings.append(f"{subtype} missing lower body mapping")
                elif subtype in ["horizontal_push", "vertical_push"] and "upper body" not in data["regions"]:
            ...
            result.success = len(result.errors) == 0 and result.details["database_stats"]["coverage_percentage"] > 50
    ...
    async def _test_enhanced_movement_integration(self, session: AsyncSession) -> TestResult:
        ...
        try:
            ...
            test_scenarios = [
                ...
                {
                    "name": "5-Day Hyrox Style",
                    "session_type": "hyrox_style",
                    "equipment": ["barbell", "dumbbell", "sled"],
                    "target_regions": ["full body"],
                    "expected_subtypes": ["squat", "hinge", "horizontal_push", "farmer_carry", "sled_push"]
                }
            ]
            ...
            for scenario in test_scenarios:
                ...
                try:
                    ...
                    coverage_percentage = len(found_subtypes.intersection(expected_coverage)) / len(expected_coverage) * 100
        ...
        try:
            ...
            for config in test_configs:
                ...
                test_request = ProgramGenerationRequest(
                    ...
                    equipment_available=config["equipment"],
    ...
    async def _test_primary_region_rotation(self, session: AsyncSession) -> TestResult:
        ...
        try:
            ...
            test_request = ProgramGenerationRequest(
                ...
                program_length_weeks=4,  # Shorter for detailed analysis
                user_level="intermediate"
            )
            ...
            if program_result.success:
                ...
                print(f"📊 Region sequence: {rotation_analysis['region_sequence']}")
                ...
                # Validate specific 5-day rotation expectations
                week1_regions = rotation_analysis["weekly_breakdown"].get(1, {})
                expected_rotation = {
                    1: ["upper"],  # Monday
                    2: ["lower"],  # Tuesday
                    4: ["upper"],  # Thursday
                ...
                for day, expected_regions in expected_rotation.items():
                    actual_regions = week1_regions.get(day, [])
                    if not any(region in str(actual_regions) for region in expected_regions):
                        rotation_matches = False
                        result.warnings.append(f"Week 1 Day {day}: Expected {expected_regions}, got {actual_regions}")
                
                if rotation_matches:
                    print("✅ 5-day rotation pattern matches expectations")
                else:
                    print("⚠️  5-day rotation pattern deviates from expectations")
    ...
    async def _test_complete_5day_program(self, session: AsyncSession) -> TestResult:
        ...
        try:
            ...
            comprehensive_request = ProgramGenerationRequest(
                ...
                availability={"days_per_week": 5},
                equipment_available=["barbell", "dumbbell", "bodyweight", "band"],
    ...
    async def _test_error_handling_integration(self) -> TestResult:
        ...
        try:
            ...
            formatted_errors = []
            for error_data in recent_errors[:2]:
                formatted_error = error_logger.format_error_for_output(error_data)
                formatted_errors.append(formatted_error)
            
            result.details["formatted_errors"] = formatted_errors
            
            print(f"✅ Logged {len(logged_errors)} test errors")
            print(f"✅ Retrieved {len(recent_errors)} recent errors")
            print(f"✅ Formatted {len(formatted_errors)} errors for output")
